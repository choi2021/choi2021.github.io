...
실제로 너무 어렵게 작업이 된 것 같다는 피드백을 받기도 했다.

해당 피드백을 받으면서 `문제를 해결하기 위해서, 필요한 기술을 사용했는데 어떤게 문제였는지` 온전히 이해하는데에는 시간이 필요했다.
...
React Native적인 작업도 좋지만 웹개발과 FE에 대한 지식을 더욱 더 깊게 배우고 싶다는 생각이 들었다. 

## 9월의 액션아이템
글을 작성하는 시점에 9월 중순이 지나가고 있지만, 9월의 액션아이템을 정리해보려한다.

- MMKV 데이터 스토리지 라이브러리 적용 작업 정리하기
- React Query를 이용한 비동기 상태 개선 작업 정리하기
- useFunnel 라이브러리 분석 후 오픈소스 분석 과정 공유하기
